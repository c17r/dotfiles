...
function __prompt_command() {
  ...
  __prompt_venv_info=()
  __prompt_venv
  if [[ "${#__prompt_venv_info[@]}" != 0 ]]; then
    PS1="$PS1$c1[$c0"
    for i in "${!__prompt_venv_info[@]}"; do
      if [[ "${__prompt_venv_info[i]}" ]]; then
        [[ $i != 0 ]] && PS1="$PS1$c1:$c0"
        PS1="$PS1\${__prompt_venv_info[$i]}"
      fi
    done
    PS1="$PS1$c1]$c9"
  fi
...
PROMPT_COMMAND="__prompt_command ; tab_title"
